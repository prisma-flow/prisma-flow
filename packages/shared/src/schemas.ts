...
export const MigrationStatusSchema = z.enum(['applied', 'pending', 'failed', 'unknown'])
export const MigrationVerificationStatusSchema = z.enum(['verified', 'unknown', 'error'])
...
export const DriftDetectionStatusSchema = z.enum(['clean', 'drifted', 'error', 'not_checked'])
...
export const DriftRepairStrategySchema = z.enum([
  'reconcile_history',
  'manual_migration',
  'manual_sql',
  'review_only',
...
export const SchemaDiffTypeSchema = z.enum([
  'model_added',
  'model_removed',
  'field_added',
  'field_removed',
...
export const SimulationVerificationSchema = z.enum(['executed', 'static-analysis', 'not-verified'])
export const SimulationOutcomeSchema = z.enum(['success', 'failure', 'unknown'])
export const SimulationModeSchema = z.enum(['static', 'shadow', 'live'])
export const SimulationStatementTypeSchema = z.enum([
  'CREATE_TABLE',
  'ALTER_TABLE',
  'DROP_TABLE',
  'CREATE_INDEX',
...
// ─── Readiness & Plan Enums ──────────────────────────────────────────────────

export const DeploymentReadinessStatusSchema = z.enum(['ready', 'attention', 'blocked'])
export const DeploymentReadinessCheckIdSchema = z.enum([
  'database',
  'drift',
...
export const DeploymentPlanPrioritySchema = z.enum(['blocker', 'recommended', 'optional'])
...
// ─── Drift Recovery / Plan-Only Schemas (V1) ─────────────────────────────────
...
export const DriftRepairPlanSchema = z.object({
  generatedAt: z.string(),
  driftCount: z.number().int().nonnegative(),
...
// ─── Schema Diff & Environment Comparison ────────────────────────────────────
...
export const EnvironmentComparisonEntrySchema = z.object({
  name: z.string(),
  reachable: z.boolean(),
...
// ─── Schema Datamodel (DMMF Explorer) ────────────────────────────────────────
...
export const SchemaDatamodelSchema = z.object({
